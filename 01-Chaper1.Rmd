...
This document is intended to collect all the useful code snippets I use.
...
Use the mtcars data where possible.
...
mtcars <- mtcars  %>% rename_all(funs(str_replace(., "_", "")))
kable(head((mtcars))) %>% kable_minimal(full_width = F)
...
**Standardize variable (mean of 0, SD of 1)**
```{r stardardise variable 1}
set.seed(1234) # randowmn seed
...
dat2 %>% summarise(across(x:z,funcs))
...
kable(glimpse(mtcars_extradata)) %>%  kable_styling(full_width = F) %>% kable_minimal()

mtcars <- left_join(mtcars,mtcars_extradata,by = 'carnames')

kable(glimpse(mtcars)) %>% kable_styling(full_width = F) %>% kable_minimal()
...
**Create a new factor from exisiting factors**
```{r}
...
```{r}
mtcars %>%  group_by(as.factor(gear)) %>%
  summarise(mean = mean(qsec), sd = sd(qsec))

#What if you wanted to run the same linear regression model by the different
#group levels?  Instead of running #summary(lm(y~x)) for the number of levels 
...
**Summarise by group**
```{r}
kable(mtcars %>% group_by(cyl) %>% summarise(Average=mean(hp), StDev=sd(hp))) %>% 
...
kable(df.sum) %>% kable_styling(full_width = FALSE) %>% kable_minimal()

df.longer <- df.sum%>% pivot_longer(col=everything(),  
names_to = c("Attribute",".value"),
  names_sep = "_")
kable(df.longer) %>% kable_styling(full_width = FALSE) %>% kable_minimal()
```
